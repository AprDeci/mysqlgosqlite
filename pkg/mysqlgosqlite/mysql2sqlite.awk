...
function extract_identifier(line,   trimmed, nameToken){
  trimmed = line
  sub(/^[ \t]+/, "", trimmed)
  if( match(trimmed, /^(`[^`]+`|"[^"]+"|[^ \t]+)/) ){
    nameToken = substr(trimmed, RSTART, RLENGTH)
    gsub(/[`"]/, "", nameToken)
    return nameToken
  }
  return ""
...
function add_id_primary_key(   pkLine ){
  if( hasIdColumn && !hasPrimaryKey && idColumnName != "" ){
    pkLine = "  PRIMARY KEY(\"" idColumnName "\")"
    if( firstInTable ){
      print pkLine
      firstInTable = 0
    }
...
function paren_delta(str,   tmp, opens, closes){
  tmp = str
  opens = gsub(/\(/, "", tmp)
  tmp = str
  closes = gsub(/\)/, "", tmp)
...
function extract_columns(def,   i, ch, depth, columnsStart, columnsEnd, columns){
  columnsStart = 0
  columnsEnd = 0
  depth = 0
  for( i = 1; i <= length(def); ++i ){
    ch = substr( def, i, 1 )
    if( ch == "(" ){
      depth++
      if( depth == 1 ){
  ...
  if( columnsStart == 0 || columnsEnd < columnsStart ){ return "" }
  columns = substr( def, columnsStart, columnsEnd - columnsStart + 1 )
  gsub( /[ \t\n\r]+/, " ", columns )
  gsub( /^ +| +$/, "", columns )
  gsub( /\([0-9]+\)/, "", columns )
  gsub( /`/, "\"", columns )
...
function reset_pending_index(){
  collectingIndex = 0
  pendingIndexBuffer = ""
  pendingIndexName = ""
  pendingIndexUnique = 0
  pendingIndexParenDepth = 0
}

function finalize_alter_index(){
  if( pendingIndexName == "" || alterTableName == "" ){ return }
  columnList = extract_columns( pendingIndexBuffer )
  if( columnList == "" ){ return }
  idxPrefix = pendingIndexUnique ? "CREATE UNIQUE INDEX" : "CREATE INDEX"
  print idxPrefix " \"idx_" alterTableName "_" pendingIndexName "\" ON \"" alterTableName "\" (" columnList ");"
  reset_pending_index()
}
...
inAlterTable != 0 {
  if( collectingIndex ){
    pendingIndexBuffer = pendingIndexBuffer " " $0
    pendingIndexParenDepth += paren_delta( $0 )
    if( pendingIndexParenDepth <= 0 ){
      finalize_alter_index()
    }
  }

  if( collectingIndex == 0 ){
    lineLower = tolower( $0 )
    if( match( lineLower, /add (primary key|unique key|key|index)/ ) ){
      if( lineLower ~ /add primary key/ ){
        # skip
      }
      else {
        pendingIndexUnique = ( lineLower ~ /add unique key/ )
        pendingIndexName = ""
        if( match( $0, /(ADD (UNIQUE )?KEY|add (unique )?key|ADD INDEX|add index) `[^`]+/ ) ){
          indexMatch = substr( $0, RSTART, RLENGTH )
          if( match( indexMatch, /`[^`]+$/ ) ){
            pendingIndexName = substr( indexMatch, RSTART+1, RLENGTH-1 )
          }
        }
        pendingIndexBuffer = $0
        pendingIndexParenDepth = paren_delta( pendingIndexBuffer )
        if( pendingIndexParenDepth <= 0 ){
          finalize_alter_index()
        }
        else {
...
/^  / && !/^(  (KEY|key)|\);)/ {
  ...
  if( tolower($0) ~ /primary key/ ){ hasPrimaryKey = 1 }
  if( idColumnName == "" ){
    candidateName = extract_identifier($0)
    if( candidateName != "" && tolower(candidateName) == "id" ){
      hasIdColumn = 1
      idColumnName = candidateName
    }
  }
